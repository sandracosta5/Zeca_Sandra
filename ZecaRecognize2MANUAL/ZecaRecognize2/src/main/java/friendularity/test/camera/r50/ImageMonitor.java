...
public class ImageMonitor extends JPanel implements Listener<ImageEvent> {
    ...
    public void handleEvent(ImageEvent t) {
        ...
        if(myImage != null){
            ...
            myImage2=ipl.getBufferedImage();
            //Gets Threshold Image - Only yellow appears as white
            IplImage detectThrs = getThresholdImage(ipl); 
            ...
            if (posX > 0 && posY > 0 && area > 500 && processContinues == true) {
                ...
                //QR Code is valid        
                if(!"QR Code não reconhecido".equals(addressQRCode)){
    ...
    public void paint(Graphics g) {
        if(myImage != null){
            g.drawImage(myImage2, 0, 0, getWidth(), getHeight(), null);
        }
    } 
     ...
     */
    private static IplImage getConvertedImage(Image myimage){
        IplImage ipl;
        ...
        ipl = IplImage.createFrom(convertedImg);
     ...
     */
    private static IplImage getThresholdImage(IplImage img){
    //converts the image in HSV
    IplImage imgHSV = cvCreateImage(cvGetSize(img), 8, 3);
    cvCvtColor(img, imgHSV, CV_BGR2HSV);
    IplImage imgThreshedBlur = cvCreateImage(cvGetSize(img), 8, 1);
    //does the Threshold between a min and a max value of yellow 
    cvInRangeS(imgHSV, rgb_min, rgb_max, imgThreshedBlur);
    //Smooths the image
    //cvSmooth (imgHSV, imgHSV, CV_BLUR, 15);
    cvSmooth (imgThreshedBlur, imgThreshedBlur, CV_BLUR, 15);
    IplImage bg = cvCreateImage(cvGetSize(imgHSV), imgHSV.depth(), imgHSV.nChannels());
    //Dilates the image
    //cvDilate(imgHSV, bg, null,6);
     ...
     */
    private static String QRReader (String add){
        Result result = null;
        BinaryBitmap binaryBitmap;
        //String in case the QR Code is not detected
        String ERROR = "QR Code não reconhecido";
        try{
